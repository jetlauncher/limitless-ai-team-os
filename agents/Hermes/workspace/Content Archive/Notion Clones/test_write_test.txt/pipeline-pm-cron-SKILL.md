...
> **Path prerequisite (HARD GATE — read first):** the two `cd …` recipes
> in steps 1 and 2 below use the canonical `$HOME/Documents/Limitless
> OS/Pipeline` form that the cron brief hardcodes. **In
> iCloud-dominant environments that path is missing while the
> iCloud mirror is alive and the system is healthy** — running the
> brief's `cd` verbatim returns nonzero, the classifier errors with
> `No such file or directory`, and a tick that escalates per the
> §"Vault-missing" decision tree will misdiagnose a healthy system as
> broken. **Always run STOP item 0's iCloud-vs-canonical detection
> FIRST, set `$PIPELINE_VAULT` to the resolved path, and use that
> for every `cd` / `python3` / `bash` / log-write in this tick.**
> The brief's `cd` is advisory once you have a resolved path
> (the `PIPELINE_VAULT="$PWD"` override on the `python3`/`bash`
> calls means the script honors whichever path you actually `cd`'d
> to). Observed receipt 2026-06-16 17:06Z: the iCloud vault was
