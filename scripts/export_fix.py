...
# Directories to skip during walk; prune before os.walk for speed on large vaults
SKIPPED = ("Skool Downloads", ".DS_Store")

def rglob_safe(base_dir: Path):
    """Walk directory tree, pruning non-doc dirs + catching OSError on iCloud drops."""
    results = []
    for root, dirs, files in os.walk(base_dir):
        # Prune heavy/non-doc directories early — this is critical on macOS with large Obsidian vaults
...
# Remove the old AGENTS line and re-add it to avoid duplication
AGENTS={'Hermes':'default','Blaze':'blaze','Bolt':'bolt','Kaijeaw':'kaijeaw','Protocol':'protocol','Qwen':'qwen','Signal':'signal','Zegna':'zegna'}
